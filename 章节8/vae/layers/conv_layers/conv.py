...
def conv2d(inp, 
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        w = tf.get_variable(name='w',
                            shape=[kernel, kernel, C, out_channel],
                            dtype=tf.float32,
                            initializer=tf.truncated_normal_initializer(
                                            mean=0.0, 
                                            stddev=math.sqrt(2 / fan_num))
                            )
...
def depthwise_conv2d(inp, 
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        w = tf.get_variable(name='w',
                            shape=[kernel, kernel, C, channel_multiplier],
                            dtype=tf.float32,
                            initializer=tf.truncated_normal_initializer(mean=0.0, stddev=math.sqrt(2 / fan_num)))
...
def depth_separable_conv2d(inp, 
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        depth_filter = tf.get_variable(name='depth_w',
                                       shape=[depth_kernel, depth_kernel, C, channel_multiplier],
                                       dtype=tf.float32,
                                       initializer=tf.truncated_normal_initializer(mean=0.0, stddev=math.sqrt(2 / depth_fan_num)))
        ...
        point_filter = tf.get_variable(name='point_w',
                                       shape=[1, 1, channel_multiplier, out_channel],
                                       dtype=tf.float32,
                                       initializer=tf.truncated_normal_initializer(mean=0.0, stddev=math.sqrt(2 / point_fan_num)))
...
def conv2d_transpose(inp, 
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable(name='w',
                            shape=[kernel, kernel, out_channel, C],
                            dtype=tf.float32,
                            initializer=tf.truncated_normal_initializer(mean=0.0, stddev=math.sqrt(2 / fan_num)))
