...
class Loss:
    @staticmethod
    def squared_difference(label, pred, name):
        with tf.variable_scope(name) as scope:
            output = tf.reduce_mean(tf.squared_difference(label, pred))
            return output

    @staticmethod
    def absolute_difference(label, pred, name):
        with tf.variable_scope(name) as scope:
    ...
    @staticmethod
    def huber_loss(label, pred, name):
        with tf.variable_scope(name) as scope:
            output = tf.losses.huber_loss(label, pred)
            return output

    @staticmethod
    def softmax_ce(label, pred, name):
        with tf.variable_scope(name) as scope:
    ...
    @staticmethod
    def hinge_loss(label, pred, name):
        with tf.variable_scope(name) as scope:
            output = tf.losses.hinge_loss(label, pred)
            return output

    @staticmethod
    def kl_div(label, pred, name):
        with tf.variable_scope(name) as scope:
            # 计算真值的熵与真值和预测值的交叉熵
